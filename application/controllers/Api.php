...
class Api extends CI_Controller
{
  ...
  public function index()
  {
    ...
    if ($q <> '') {
      $config['base_url'] = base_url() . 'api/index.html?q=' . urlencode($q);
      $config['first_url'] = base_url() . 'api/index.html?q=' . urlencode($q);
    } else {
      $config['base_url'] = base_url() . 'api/index.html';
      $config['first_url'] = base_url() . 'api/index.html';
    }

    $config['per_page'] = 10;
    $config['page_query_string'] = TRUE;
    $config['total_rows'] = $this->Api_model->total_rows($q);
    $post = $this->Api_model->get_limit_data($config['per_page'], $start, $q);
  ...
  {
    $row = $this->Api_model->get_by_id($id);
    if ($row) {
      $data = array(
        'id_post' => $row->id_post,
      ...
      $data['pageTitle'] = 'Detail Api';
      $this->load->view('theme/tpl_header',$data);
      $this->load->view('api/api_read', $data);
      $this->load->view('theme/tpl_footer');            
    } else {
  ...
  public function create() 
  {
    ...
    $data = array(
      'pageTitle' => 'New Api',
      'button' => 'Terbitkan',
      'list_category' => $this->Api_model->get_category(),
      'list_tag' => $this->Api_model->get_tag(),
      'action' => site_url('api/create_action'),
    ...
    $this->load->view('theme/tpl_header',$data);
    $this->load->view('api/api_form', $data);
    $this->load->view('theme/tpl_footer');          
  }
  ...
  public function update($id) 
  {
    ...
    if ($row) {
      $data = array(
        ...
        'list_category' => $this->Api_model->get_category(),
        'list_tag' => $this->Api_model->get_tag(),        
        'action' => site_url('api/update_action'),
  ...
  public function update_action() 
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $this->Api_model->update($this->input->post('id_post', TRUE), $data);
 ...
 public function excel()
 {
  ...
  foreach ($this->Api_model->get_all() as $data) {
    $kolombody = 0;
...
public function word()
{
  ...
  $data = array(
    'api_data' => $this->Api_model->get_all(),
    'start' => 0
    );
...
{  
 if(!($_POST["title"]))  
 {  
  echo '<span class="text text-danger"><i class="fa fa-remove"></i> Sepertinya API yang anda Buat Sudah ada Sebelumnya</span></span>';  
}  
else  
{  
  $this->load->model("Api_model");  
  if($this->Api_model->unique($_POST["title"]))  
  {  
   echo '<span class="text text-danger"><i class="fa fa-remove"></i> Sepertinya API yang anda Buat Sudah ada Sebelumnya</span>';  
 }  
 else  
 {  
   echo '<span class="text text-success"><i class="fa fa-check"></i> API Dapat Dibuat</span>';  
 }  
}  
}  
...
public function image($id) 
{
  ...
  if ($row) {
    $data = array(
      ...
      'button' => 'Edit Image',
      'action' => site_url('api/image_action/'.$row->url),
      'id_post' => set_value('id_post', $row->id_post),
      'image' => set_value('image', $row->image),
      );

    $this->load->view('theme/tpl_header',$data);
    $this->load->view('api/api_form_image', $data);
    $this->load->view('theme/tpl_footer');  

  } else {

    $this->session->set_flashdata('message', 'Record Not Found');
    redirect(site_url('api/image'));
...
{  
  ...
  {  
    $config['upload_path'] = './assets/uploads/api/big/'; 
    $config['allowed_types'] = 'jpg|jpeg|png|gif';  
   ...
   {  
     ...
     $config['image_library'] = 'gd2';  
     $config['source_image'] = './assets/uploads/api/big/'.$data["file_name"];  
     $config['create_thumb'] = FALSE;  
     $config['maintain_ratio'] = FALSE;  
     $config['quality'] = '40%';  
     $config['width'] = 200;  
     $config['height'] = 200;  
     $config['new_image'] = './assets/uploads/api/middle/'.$data["file_name"];  
     $this->load->library('image_lib', $config);  
     ...
     $this->Api_model->update($id, $update);

     echo '<img src="'.base_url().'./assets/uploads/api/middle/'.$data["file_name"].'" class="img-thumbnail" />';  
   }  
 }  
